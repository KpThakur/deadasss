...
import {
    View, Text, Image, TouchableOpacity, FlatList, ScrollView, StatusBar
} from 'react-native';
import moment from 'moment'
import Button from '../../../../Components/Button';
import {FONT_FAMILY_SFU_REGULAR, CHANGE_PASSWORD_COLOUR_CODE, WHITE_COLOR_CODE, BLACK_COLOUR_CODE } from '../../../../Utils/constant';
        ...
        <View style={styles.container}>
            <View style={styles.body}>
                ...
                <View style={{ flex: 1,paddingTop:25 }}>
                    <View style={{ justifyContent: 'space-between', alignItems: 'center', paddingTop: Platform.OS === 'ios' ?20 :5, flexDirection: 'row' }}>
                        <View style={{ width: '20%' }} />
                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' , }}>
                            <Image style={{ height: 70 ,resizeMode:'contain'}} source={require('../../../../Assets/deadasss.png')} />
                        </View>
                        <View style={{ width: '20%',flexDirection:'row',justifyContent:'flex-end' }}>
                            <TouchableOpacity onPress={() => props.onPressCross()} style={{ paddingRight:25}}>
                    ...
                    <View style={{ flex: 1 }}>
                        <View style={{ justifyContent: 'center', height: 50,alignItems:'center' }}>
                            <Text style={{ fontSize: 20, color: '#fff', paddingLeft: 25, fontFamily: FONT_FAMILY_SFU_REGULAR, }}>Account Balance and History</Text>
                        </View>
                        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                            <Text style={{ fontSize: 22, color: '#fff', fontFamily: FONT_FAMILY_SFU_REGULAR, }}>{props.userWalletDetails ? props.userWalletDetails.first_name : null}</Text>
                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingTop: 20, padding: 15 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Image source={require('../../../../Assets/wallet.png')} style={{ height: 40, width: 40 }} />
                                <View style={{ paddingLeft: 15 }}>
                                    <Text style={{ fontSize: 18, color: "#fff", fontFamily: FONT_FAMILY_SFU_REGULAR, }}>Balance</Text>
                                </View>
                            </View>
                            <View style={{ paddingRight: 20, justifyContent: 'center' }}>
                                <Text style={{ fontSize: 15, color: '#fff', fontFamily: FONT_FAMILY_SFU_REGULAR, }}>$ {props.userWalletDetails ? props.userWalletDetails.user_wallet : null}</Text>
                            </View>
                        </View>
                        {props.userWalletDetails?.user_wallet == 0 ?
                            <View style={{ height: 80, justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                                <View style={{
                                    ...
                                    <Text style={{ color: '#fff', fontFamily: FONT_FAMILY_SFU_REGULAR, fontSize: 15 }}>Withdrawal</Text>
                                </View>
                            </View>
                            :
                            <View style={{ height: 80, justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                                <TouchableOpacity onPress={() => props.WithdrawalFun()} style={{
                                    backgroundColor: BLACK_COLOUR_CODE,
                                    height: 50,
                                    borderRadius: 5,
                                    marginLeft: 20,
                                ...
                                showsVerticalScrollIndicator={false}
                                    ...
                                    renderItem={({ item, index }) =>
                                        !item.payment_amount == 0 ?
                                            <View key={index} style={{ flexDirection: 'row', justifyContent: 'space-between', height: 60 }}>
                                                <View style={{ flexDirection: 'row', width: '80%', alignItems: 'center' }}>
                                                    <View style={{ width: 60 }}>
                                                        <Image source={require('../../../../Assets/money-transfer.png')} style={{ height: 40, width: 40 }} />
                                                    </View>
                                                    <View>
                                                        <Text style={{ color: '#fff', fontFamily: FONT_FAMILY_SFU_REGULAR, fontSize: 15 }}>{item.payment_status == 4 ? item.challenge_title : item.amount_pay_by}</Text>
                                                        <Text style={{ fontSize: 15, color: '#fff', fontFamily: FONT_FAMILY_SFU_REGULAR, }}>{moment(item.create_date).format('DD MMM YYYY')}</Text>
                                                    </View>
                                                </View>
                                                <View style={{ paddingRight: 20, justifyContent: 'center' }}>
                                                    <Text style={{ fontSize: 15, color: item.payment_status == 4 ? '#3124F4' : item.payment_status == 3 ? '#F23C23' : '#fff', fontFamily: FONT_FAMILY_SFU_REGULAR, }}>{item.payment_status == 3 ? '-' : null} $ {item.payment_amount}</Text>
                                                </View>
                                            </View>
                                            :
                                            null
