...
export default class JoinChannelAudio extends Component<{}, State, any> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      roomID: this.props.route.params ? this.props.route.params.data.room_id : 'guest',
      payToId: this.props.route.params ? this.props.route.params.data.pay_to_id : 'guest',
      challengeId: this.props.route.params ? this.props.route.params.data.challenge_id : 'guest',
      remaningTime: props.route.params ? props.route.params.data.call_remaining_duration : '900',
      isJoined: false,
      audMute: false,
      remoteUid: [],
      switchCamera: true,
  ...
  storeDataLocal = async () => {
    ...
    await AsyncStorage.setItem('challengeId', JSON.stringify(this.state.challengeId))
  }
  ...
  callNotRecievFun = async () => {
    try {
      const params = {
        "room_id": (this.props.route.params.Status === 1) ? this.props.route.params.remoteMessage.data.room_id : this.props.route.params.remoteMessage.room_id,
        "challenge_id": (this.props.route.params.Status === 1) ? this.props.route.params.remoteMessage.data.challenge_id : this.props.route.params.remoteMessage.challenge_id,
        "call_status": 5,
      }
  ...
  _handleCallStatus = async (interval) => {
    try {
      const params = {
        "room_id": (this.props.route.params.Status === 1) ? this.props.route.params.remoteMessage.data.room_id : this.props.route.params.remoteMessage.room_id,
        "pay_to_id": (this.props.route.params.Status === 1) ? this.props.route.params.remoteMessage.data.pay_to_id : this.props.route.params.remoteMessage.pay_to_id,
      }
      const { data } = await apiCall('POST', ENDPOINTS.GET_CALL_STATUS, params);
      if (data.status === 200) {
  ...
  _joinChannel = async () => {
    await this._engine?.joinChannel(this.props.route.params.data.agora_token, this.state.channelId, null, 0)
  };
      ...
      <View style={styles.container}>
        ...
        <View style={{ position: 'absolute', bottom: 50, width: '100%', marginRight: 15 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <TouchableOpacity onPress={() => this._switchCamera()} style={{ height: 50, width: 50, borderRadius: 50, backgroundColor: '#a9a9a9', marginRight: 15, justifyContent: 'center', alignItems: 'center' }}>
              <Image source={require('./assets/dslr-camera.png')} style={{ width: 30, height: 30 }} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => this.cancelCall()} style={{ height: 50, width: 50, marginRight: 15, borderRadius: 50, backgroundColor: 'red', justifyContent: 'center', alignItems: 'center' }}>
              <Image source={require('../../../Assets/call_discontinue_icon.png')} style={{ width: 30, height: 30 }} />

            </TouchableOpacity>
            <TouchableOpacity onPress={() => this.toggleAudio()} style={{ height: 50, width: 50, borderRadius: 50, backgroundColor: this.state.audMute ? '#B0DAF2' : '#a9a9a9', justifyContent: 'center', alignItems: 'center' }}>
              <Image source={require('./assets/mute-microphone.png')} style={{ width: 30, height: 30 }} />
            </TouchableOpacity>
          </View>
        </View>
        <KeepAwake />
      ...
      <View style={styles.container}>
        {
          this.state.CameraShow == 'big' ?
            ...
            :
            <TouchableOpacity onPress={() => this.setState({ CameraShow: 'big' })} style={{
              position: 'absolute',
              zIndex: 2,
              width: 110,
              height: 120,
              ...
              <RtcLocalView.SurfaceView style={{
                position: 'absolute',
                // zIndex: 2,
                width: 110,
                height: 120,
        ...
        {this.state.CameraShow == 'big' ?
          <TouchableOpacity onPress={() => this.setState({ CameraShow: 'small' })} style={{
            position: 'absolute',
            zIndex: 2,
            width: 110,
            height: 120,
