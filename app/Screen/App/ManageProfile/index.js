    ...
    async function _handleUpdateProfile(parameters) {
        ...
        if (valid) {
            try {
                ...
                parameters.filepath.path && formdata.append('profile_pic', {
                    uri: parameters.filepath.path,
                    type: parameters.filepath.mime,
                    name: parameters.filepath.path.substring(parameters.filepath.path.lastIndexOf('/') + 1)
                });
                const { data } = await apiCall('POST', ENDPOINTS.UPDATE_PROFILE, formdata);
                if (data.status === 200) {
                    setIsLoading(false)
