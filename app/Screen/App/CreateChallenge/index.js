...
const CreateChallengeView = () => {
    ...
    async function onPressCreateChallenge() {
        ...
        try {
            ...
            setIsLoading(true)
            const { data } = await apiCall('POST', ENDPOINTS.CREATE_CHALLENGE, params);
            if (data.status === 200) {
                ...
                setChallengeTitle('')
                setPriceCovert('')
                await AsyncStorage.setItem('challengeData', JSON.stringify(data.data));
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <CreateChallenge
                ...
                userData={userData}
                userAmout={(selectNumber * priceCovert) / 100 * userData.commission_amount}
            />
            <AnimatedAlert
