...
const SettingChangePassword = () => {
  ...
  const validationform = parameters => {
    if (parameters.OldPassword === '') {
      AnimatedAlert.showAlert();
      setAlertMessage('Please enter old password');
      return false;
    }
    ...
    if (parameters.NewPassword.length <= 5) {
        AnimatedAlert.showAlert()
    ...
    if (parameters.ConfirmPassword !== parameters.NewPassword) {
      AnimatedAlert.showAlert();
      setAlertMessage("Confirm password and new password doesn't match ");
      return false;
    }
    return true;
  };
